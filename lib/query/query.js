 ...
 * @param {Function} [options.handleMissingVars] - If provided, this function is called when a $var
 *   substitution is encountered for a variable not present in `vars`.  It is called with the
 *   missing variable name and should return the value to substitute in its place.  If not
 *   provided, encountering a missing variable throws a MissingQueryVarError.
 */
class Query {
	...
	constructor(queryData, queryFactory, options = {}) {
		...
		if (options.vars) this._substituteVars(options.vars, options.handleMissingVars);
	 ...
	 * @param {Function} [handleMissing] - If provided, called with the missing variable name
	 * when a $var has no entry in `vars`; its return value is substituted in.  If not
	 * provided, a MissingQueryVarError is thrown for missing variables.
	 */
	_substituteVars(vars, handleMissing) {
		...
		function subst(obj) {
			if (Query.isVarExpression(obj)) {
				...
				if (varValue === undefined) {
					if (typeof handleMissing === 'function') {
						return handleMissing(varName);
					} else {
